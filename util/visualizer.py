...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        # save melspec and synthesize wavefile
        mel_spec = np.squeeze(im, 0)
        out_file_path = '%s/%s_%s.npz' % (image_dir, name, label)
        feats = {}
        feats['mel_spec'] = mel_spec
        np.savez(out_file_path, **feats)
        wav = audio.inv_mel_spectrogram(mel_spec)
        wav_path = '%s/%s_%s.wav' % (image_dir, name, label)
